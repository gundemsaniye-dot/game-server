...
const markerGids = { bridge: 577, blocked: 578, cost2: 579, cost3: 580, cost4: 581 };
const markerTilesets = [
  ...
  { firstgid: markerGids.cost2, source: "../tilesets/navigation-cost.tsj" },
];
...
const costGids = new Set([markerGids.cost2, markerGids.cost3, markerGids.cost4]);
...
for (const file of files) {
  ...
  const costs = layer(map, "NAV_COST");
  const bridges = layer(map, "05_BRIDGES");
  for (let index = 0; index < blocked.data.length; index += 1) {
    const authored = [blocked.data[index], costs.data[index], bridges.data[index]];
    ...
    costs.data[index] = authored.find((gid) => costGids.has(gid)) ?? 0;
  }
  ...
  for (let index = 0; index < blocked.data.length; index += 1) {
    if (blocked.data[index] === markerGids.blocked) {
      costs.data[index] = 0;
    }
  }
  Object.assign(blocked, { visible: true, opacity: 0.55 });
  Object.assign(costs, { visible: true, opacity: 0.45 });
  ...
  const objectsIndex = map.layers.findIndex((candidate) => candidate.name === "OBJECTS_BELOW");
